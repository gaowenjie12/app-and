			...
			try {
				for (String file : files) {
					try {
                       ...
                       android.util. Log.i("onion", "SQLiteException" + e.toString());
...
//                        "http://192.168.1.66:9095" + "/api/v1/communities/"+PreferencesUtil.getCommityId(context)+"/crash/",
                                Config.NET_AGENT + "/api/v1/communities/" + PreferencesUtil.getCommityId(XjApplication.getInstance()) + "/crash/",
//                        info);
                              MyCrashHandler.getErrorInfoJSONString(XjApplication.getInstance()).toString());*/
                    }
				}
				db.setTransactionSuccessful();
			}
