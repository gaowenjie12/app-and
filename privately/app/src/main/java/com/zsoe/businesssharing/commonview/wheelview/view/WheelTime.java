package com.zsoe.businesssharing.commonview.wheelview.view;

import android.view.View;

import com.zsoe.businesssharing.R;
import com.zsoe.businesssharing.commonview.wheelview.adapter.ArrayWheelAdapter;
import com.zsoe.businesssharing.commonview.wheelview.adapter.NumericMinutesWheelAdapter;
import com.zsoe.businesssharing.commonview.wheelview.adapter.NumericWheelAdapter;
import com.zsoe.businesssharing.commonview.wheelview.listener.ISelectTimeCallback;
import com.zsoe.businesssharing.commonview.wheelview.listener.OnItemSelectedListener;
import com.zsoe.businesssharing.commonview.wheelview.utils.ChinaDate;
import com.zsoe.businesssharing.commonview.wheelview.utils.LunarCalendar;
...
public class WheelTime {
    ...
    private int endtHour = DEFAULT_END_HOUR;

    private int currentYear;

    private boolean isRangTime = false;//是否设置时间范围
    ...
    private List<String> minutesValueList;

    public void setPicker(int year, final int month, int day, int h, int m, int s) {

        minutesValueList = new ArrayList<>();
        minutesValueList.add("00");
        minutesValueList.add("30");
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        //TODO NumericMinutesWheelAdapter 是为了满足需求，自定义的。使之能够选择 00 和 30 其中一个
        wv_minutes.setAdapter(new NumericMinutesWheelAdapter(minutesValueList));
...
//        wv_hours.setAdapter(new NumericWheelAdapter(0, 23));
        ...
        wv_hours.setAdapter(new NumericWheelAdapter(startHour, endtHour));
        if (isRangTime) {
            //如果设置了时间范围，从第一个时间点开始显示
            wv_hours.setCurrentItem(0);
        } else {
            //如果没有设置了时间范围，从现在时间点开始显示
            wv_hours.setCurrentItem(h);
        }
        ...
        //TODO NumericMinutesWheelAdapter 是为了满足需求，自定义的。使之能够选择 00 和 30 其中一个
        if (isRangTime && isToday) {
            //如果设置了时间范围，第一条分钟数据显示计算好的，有可能是[00] 或者是 [00,30]
            wv_minutes.setAdapter(new NumericMinutesWheelAdapter(startMinuteValueList));
        } else {
            //如果没有设置了时间范围，第一条分钟数据显示计算[00,30]
            wv_minutes.setAdapter(new NumericMinutesWheelAdapter(minutesValueList));
        }
        ...
        if (isRangTime) {
            wv_hours.setOnItemSelectedListener(new OnItemSelectedListener() {
                ...
                public void onItemSelected(int index) {
                    ...
                    wv_minutes.setAdapter(null);
                    //如果做了时间范围的限定，并且是第一个适配器。则设置计算好的开始分钟，有可能是 00 或者是 [00，30]
                    if (index == 0 && isToday) {
                        wv_minutes.setAdapter(new NumericMinutesWheelAdapter(startMinuteValueList));
                    } else {
                        //如果不是第一个适配器，则放入通用的时间集合，即 [00，30]
                        wv_minutes.setAdapter(new NumericMinutesWheelAdapter(minutesValueList));
                    }
...
//                maxItem = endD;
           ...
           /* int i = wv_month.getCurrentItem() + startMonth;
            ...
            if ((wv_month.getCurrentItem() + startMonth) == startMonth) {
                sb.append((wv_year.getCurrentItem() + startYear)).append("-")
                        ...
                        .append(wv_hours.getCurrentHourItem()).append(":")
                        .append(wv_minutes.getCurrentMinuteItem()).append(":")
    ...
    public void setRangTime(boolean isRangTime) {
        this.isRangTime = isRangTime;
    }
    ...
    public void setRangDate(Calendar startDate, Calendar endDate) {
        ...
        if (isRangTime) {
            ...
            if (startDate != null && endDate != null) {
                ...
                endtHour = endDate.get(Calendar.HOUR_OF_DAY);
            } else if (startDate == null && endDate != null) {
                endtHour = endDate.get(Calendar.HOUR_OF_DAY);
            } else if (startDate != null && endDate == null) {
                startHour = startDate.get(Calendar.HOUR_OF_DAY);
            ...
            //计算开始时间是否是从00分开始，如果从00分开始，则包括[00，30]
            String minutes = DateUtil.getMinutes(startDate);
            if (minutes.equals("00")) {
                startMinuteValueList.add("00");
                startMinuteValueList.add("30");
    ...
    private List<String> startMinuteValueList = new ArrayList<>();
