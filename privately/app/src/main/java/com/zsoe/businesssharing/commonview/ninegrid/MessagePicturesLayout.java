...
public class MessagePicturesLayout extends FrameLayout implements View.OnClickListener {
    ...
    private List<ImageInfo> mThumbDataList;
    ...
    public MessagePicturesLayout(Context context, @Nullable AttributeSet attrs) {
        ...
        mSpace = (int) (TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 8, mDisplayMetrics) + 0.5f);
    ...
    public void set(List<ImageInfo> urlThumbList, List<ImageInfo> urlList) {
    ...
    private void notifyDataChanged() {
        final List<ImageInfo> thumbList = mThumbDataList;
...
//                (int) ((getWidth() * 1f - mSpace * (column - 1)) / column);
        ...
        for (int i = 0; i < iPictureList.size(); i++) {
            ...
            if (i < urlListSize) {
                ...
                GlideUtils.loadImage(getContext(), thumbList.get(i).getPicture(), iPicture);

                if (urlListSize == 1) {
                    //如果是一个图片,靠最右侧显示
                    iPicture.setTranslationX(2 * (imageSize + mSpace));
                } else if (urlListSize == 2) {
                    //如果是两张图片，第二张在最右侧显示，第一张在中间显示
                    if (i == 0) {
                        iPicture.setTranslationX(1 * (imageSize + mSpace));
                    } else if (i == 1) {
                        iPicture.setTranslationX(2 * (imageSize + mSpace));
                    }
                } else {
    ...
    public interface Callback {
        void onThumbPictureClick(ImageView i, SparseArray<ImageView> imageGroupList, List<ImageInfo> urlList);
    }
