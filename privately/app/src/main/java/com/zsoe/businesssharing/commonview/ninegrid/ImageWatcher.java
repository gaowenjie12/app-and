...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    protected int mErrorImageRes = R.mipmap.icon_default; // 图片加载失败站位图
    ...
    protected List<ImageInfo> initUrlList;
    ...
    protected List<ImageInfo> mUrlList; // 图片地址列表
    ...
    public interface IndexProvider {
        ...
        void onPageChanged(ImageWatcher imageWatcher, int position, List<ImageInfo> dataList);
    }
    ...
    public interface OnPictureLongPressListener {
         ...
         */
        void onPictureLongPress(ImageView v, ImageInfo uri, int pos); // 当前展示图片长按的回调
    }

    public void show(List<ImageInfo> urlList, int initPos) {
        if (urlList == null) {
     ...
     */
    public void show(ImageView i, SparseArray<ImageView> imageGroupList, final List<ImageInfo> urlList) {
    ...
    private void showInternal(ImageView i, SparseArray<ImageView> imageGroupList, final List<ImageInfo> urlList) {
        if (loader == null) {
    ...
    public String getDisplayingUri() {
        return mUrlList != null ? mUrlList.get(getCurrentPosition()).getPicture() : null;
    }
    ...
    class ImagePagerAdapter extends PagerAdapter {
        ...
        private boolean setDefaultDisplayConfigs(final ImageView imageView, final int pos, boolean hasPlayBeginAnimation) {
            ...
            loader.load(imageView.getContext(), mUrlList.get(pos).getPicture(), new LoadCallback() {
                @Override
