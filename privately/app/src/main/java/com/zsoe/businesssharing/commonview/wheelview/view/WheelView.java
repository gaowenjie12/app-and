...
import com.zsoe.businesssharing.commonview.wheelview.IPickerViewData;
import com.zsoe.businesssharing.commonview.wheelview.adapter.WheelAdapter;
import com.zsoe.businesssharing.commonview.wheelview.listener.LoopViewGestureListener;
import com.zsoe.businesssharing.commonview.wheelview.listener.OnItemSelectedListener;
import com.zsoe.businesssharing.commonview.wheelview.timer.InertiaTimerTask;
import com.zsoe.businesssharing.commonview.wheelview.timer.MessageHandler;
import com.zsoe.businesssharing.commonview.wheelview.timer.SmoothScrollTimerTask;
...
public class WheelView extends View {
    ...
    /**
     * 为满足业务需求，添加了一个获取小时 时间点内容的 方法。 于长亮添加
     */
    public final int getCurrentHourItem() {
        // return selectedItem;
        if (adapter == null) {
            return 0;
        }
        ...
        return (Integer) adapter.getItem(Math.min(selectedItem, adapter.getItemsCount() - 1));
    }

    /**
     * 为满足业务需求，添加了一个获取分钟 方法。于长亮添加
     */
    public final int getCurrentMinuteItem() {
        // return selectedItem;
        if (adapter == null) {
            return 0;
        }
        if (isLoop && (selectedItem < 0 || selectedItem >= adapter.getItemsCount())) {
            int max = Math.max(0, Math.min(Math.abs(Math.abs(selectedItem) - adapter.getItemsCount()), adapter.getItemsCount() - 1));
            return (int) adapter.getItem(max);
        }

        int max = Math.max(0, Math.min(selectedItem, adapter.getItemsCount() - 1));
