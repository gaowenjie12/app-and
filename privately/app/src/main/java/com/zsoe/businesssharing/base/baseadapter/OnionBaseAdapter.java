        ...
        for (String fieldName : vh.findImgs.keySet()) {
            ...
            if (picisNative) {//加载本地图片
                ...
                if (id != 0) {//本地图片存在
                    ...
                    AppImageUtils.displayImage(context, vh.findImgs.get(fieldName),"");
//                        ImageLoader.getInstance().displayImage("", vh.findImgs.get(fieldName), displayImageOptions);
             }
            } else {
                AppImageUtils.displayImage(context,vh.findImgs.get(fieldName),text);
            }
        }
                ...
                for (Field f : fields) {
                    ...
                    if (textField != null && convertView.findViewById(textField.value()) != null) {
                        ...
                        if (imageField != null && convertView.findViewById(imageField.value()) != null) {
                            ...
                            findImgs.put(f.getName(), (SimpleDraweeView) convertView.findViewById(vId));
                        }
                    }
                }
            }
